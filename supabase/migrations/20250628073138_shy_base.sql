/*
  # Complete RentSnap Database Schema Migration
...
CREATE TABLE IF NOT EXISTS user_profiles (
  ...
  avatar_url text,
  user_type user_type NOT NULL DEFAULT 'tenant',
  phone text,
  bio text,
...
DROP POLICY IF EXISTS "Landlords can update property images" ON storage.objects;
DROP POLICY IF EXISTS "Landlords can delete property images" ON storage.objects;
...
CREATE POLICY "Landlords can update property images" ON storage.objects
  FOR UPDATE USING (
    bucket_id = 'property-images' AND 
    EXISTS (
      SELECT 1 FROM properties 
